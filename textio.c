...
void parprocess(MPI_File *in, MPI_File *out, const int rank, const int size,
                const int overlap) {
   ...
   */
  {
    ...
    MPI_File_read_at_all(*in, globalstart, chunk, mysize, MPI_CHAR,
                         MPI_STATUS_IGNORE);
    chunk[mysize] = '\0';
  }
  ...
  MPI_File_write_at_all(*out, (MPI_Offset)(globalstart + (MPI_Offset)locstart),
                        &(chunk[locstart]), mysize, MPI_CHAR,
                        MPI_STATUS_IGNORE);
...
int main(int argc, char **argv) {
  ...
  ierr = MPI_File_open(MPI_COMM_WORLD, argv[1], MPI_MODE_RDONLY, MPI_INFO_NULL,
                       &in);
  if (ierr) {
    if (rank == 0)
  ...
  ierr = MPI_File_open(MPI_COMM_WORLD, argv[2],
